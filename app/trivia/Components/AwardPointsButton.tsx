...
export default function AwardPointsButton(props: IAwardPointsButtonProps) {
  ...
  return (
    <>
      <button className="btn-yellow" popoverTarget="awardPointsPopover">
        Award
      </button>
      <div
        ref={ref}
        popover="auto"
        id="awardPointsPopover"
        className="bg-black text-white m-auto w-[80%] md:w-[40%] max-h-[70%] shadow-md border-1 border-slate-700 p-4"
      >
        <div className="flex flex-col gap-4">
          <div className="text-center">
            Award <span className="text-lg mx-1">{points}</span> points to{" "}
            <span className="mx-1 text-lg">{player}</span>
          </div>
          <label>Points</label>
          <div className="flex gap-3 flex-wrap justify-evenly">
            {possiblePoints.map((p) => (
              <button key={p} className="btn-blue" onClick={() => setPoints(p)}>
                {p}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2 flex-wrap justify-evenly">
            {players?.map((p) => (
              <button
                key={p.name}
                className="w-[30%] break-all btn-green"
